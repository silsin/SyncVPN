...
using SyncVPN.Crypto.Contracts.Extensions;
...
public class GlobalSettingsMigrator : IGlobalSettingsMigrator
{
    ...
    public string? GetLegacyUserConfigFilePath()
    {
        ...
        // v3
        string? latestVersionFolderPath = GetOldUserConfigPathByFolderPrefix(appDataFolderPath, "SyncVPN_Url_");
        if (string.IsNullOrEmpty(latestVersionFolderPath))
        {
            // v2
            latestVersionFolderPath = GetOldUserConfigPathByFolderPrefix(appDataFolderPath, "SyncVPN.exe_Url_");
        }
    ...
    {
        string xPath = $"/configuration/userSettings/SyncVPN.Properties.Settings/setting[@name='{name}']/value";
