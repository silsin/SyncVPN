...
using SyncVPN.Logging.Contracts.Events.TelemetryLogs;
using SyncVPN.StatisticalEvents.Files;
using SingleAction = SyncVPN.Common.Core.Threading.SingleAction;
...
public abstract class StatisticEventSenderBase
{
    ...
    public async Task EnqueueAsync(StatisticalEvent statisticalEvent)
    {
        ...
        try
        {
            ...
            Logger.Debug<TelemetryLog>($"Statistical event queued. {JsonConvert.SerializeObject(statisticalEvent)}");
            if (EventsToSend.Count > MAX_NUM_OF_EVENTS)
            {
                int numOfEventsToDelete = EventsToSend.Count - MAX_NUM_OF_EVENTS;
                Logger.Warn<TelemetryLog>($"Too many statistical events. Deleting the {numOfEventsToDelete} oldest" +
                ...
                for (int i = 0; i < numOfEventsToDelete; i++)
                {
                    ...
                    Logger.Debug<TelemetryLog>($"Statistical event deleted. {JsonConvert.SerializeObject(deletedStatisticalEvent)}");
                }
            }
            Logger.Info<TelemetryLog>($"{EventsToSend.Count} statistical events are now queued.");
    ...
    private async Task SendAsync()
    {
        ...
        try
        {
            ...
            Logger.Info<TelemetryLog>($"Sending {numOfEvents} statistical events.");
            ApiResponseResult<BaseResponse> baseResponse = await SendApiRequestAsync(statisticalEventsBatch);
            if (baseResponse.Success)
            {
                Logger.Info<TelemetryLog>($"Successfully sent {numOfEvents} statistical events. Removing them from the queue.");
                await RemoveSuccessfullySentEventsAsync(statisticalEventsBatch.EventInfo);
            }
            else
            {
                Logger.Error<TelemetryLog>($"Failed to send {numOfEvents} statistical events. Keeping them in the queue.");
            }
        }
        catch (Exception ex)
        {
            Logger.Error<TelemetryLog>($"Exception thrown when sending {numOfEvents} statistical events. Keeping them in the queue.", ex);
        }
    }
    ...
    private async Task RemoveSuccessfullySentEventsAsync(List<StatisticalEvent> statisticalEventsSent)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < numOfEventsSent; i++)
            {
                ...
                {
                    Logger.Warn<TelemetryLog>($"Can't remove statistical events because the queue is null.");
                    break;
                }
                if (!EventsToSend.TryDequeue(out StatisticalEvent? statisticalEvent))
                {
                    Logger.Warn<TelemetryLog>($"The statistical events queue is unexpectedly shorter. Occurred when removing {i} of {numOfEventsSent}.");
                    break;
                }
            }
            if (EventsToSend is not null)
            {
                Logger.Info<TelemetryLog>($"Removed successfully sent statistical events. {EventsToSend.Count} events are now queued.");
            }
            SaveToFile(EventsToSend?.ToList() ?? []);
        }
        finally
    ...
    protected void ClearEventsDueToDisabledTelemetry()
    {
        if (EventsToSend is null)
        {
            Logger.Warn<TelemetryLog>($"Can't clear statistical events because the queue is null.");
        }
        else
        {
            ...
            EventsToSend.Clear();
            SaveToFile(EventsToSend.ToList());
            Logger.Info<TelemetryLog>($"Statistical events cleared from telemetry becoming disabled. " +
