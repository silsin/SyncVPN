...
public class SettingsHeartbeatReporter : ReporterBase<SettingsMeasurementGroup>,
    ISettingsHeartbeatReporter
{
    private readonly ISettingsHeartbeatDimensionsBuilder _dimensionsBuilder;

    public override string Event => "settings_heartbeat";

    public SettingsHeartbeatReporter(
        ISettingsHeartbeatDimensionsBuilder dimensionsBuilder,
    ...
    {
        return ReportEventAsync(
            CreateStatisticalEventBuilder()
                .WithDimensions(_dimensionsBuilder.Build())
                .Build());
    }
}