...
public struct VpnHost
{
    ...
    // to fetch the credentials themselves, unlike the legacy backend's separately-fetched, cacheable server list -
    // so there is no separate tampering risk for the legacy backend's Ed25519 server-list signature to guard against,
