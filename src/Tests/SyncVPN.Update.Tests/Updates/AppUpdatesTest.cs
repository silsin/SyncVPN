...
namespace SyncVPN.Update.Tests.Updates
{
    [TestClass]
    public class AppUpdatesTest
    {
        ...
        public void Cleanup_ShouldDelete_AllNotExeFiles_FromDownloadsDirectory()
        {
            ...
            CopyFile("SyncVPN_win_v1.0.0.exe", updatesPath, "Some.1");
            CopyFile("SyncVPN_win_v2.0.0.exe", updatesPath, "Later version not exe.dll");
        ...
        public void Cleanup_ShouldDelete_OutdatedExeFiles_FromDownloadsDirectory()
        {
            ...
            CopyFile("SyncVPN_win_v1.0.0.exe", updatesPath);
            CopyFile("SyncVPN_win_v1.5.0.exe", updatesPath);
            CopyFile("SyncVPN_win_v1.5.1.exe", updatesPath);
            ...
            files.Should()
                .HaveCount(1)
                .And.Match(f => Path.GetFileName(f.First()) == "SyncVPN_win_v1.5.1.exe");
        }
