...
namespace SyncVPN.Update.Tests.Updates
{
    ...
    public class AppUpdateTest
    {
        ...
        public async Task Ready_ShouldBeTrue_AfterValidated_WhenFileAlreadyExists()
        {
            ...
            CopyFile("SyncVPN_win_v1.5.1.exe", TestConfig.GetFolderPath(downloadsPath));
            update = await update.Validated();
            File.Delete(Path.Combine(TestConfig.GetFolderPath(downloadsPath), "SyncVPN_win_v1.5.1.exe"));
