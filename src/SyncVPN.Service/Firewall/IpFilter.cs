...
public class IpFilter : IStartable
{
    ...
    private void CreateDynamicSession()
    {
        DynamicInstance = NetworkFilter.IpFilter.Create(
            Session.Dynamic(),
            new DisplayData { Name = "SyncVPN Dynamic Provider" });

        DynamicSublayer = DynamicInstance.CreateSublayer(new DisplayData { Name = "SyncVPN Dynamic Sublayer" },
        ...
        try
        {
            ExecuteTransaction(session, () =>
            {
                NetworkFilter.IpFilter instance = NetworkFilter.IpFilter.Create(session,
                    new DisplayData { Name = "SyncVPN Permanent Provider" },
                    true,
                    _providerGuid);

                instance.CreateSublayer(new DisplayData { Name = "SyncVPN Permanent Sublayer" },
    ...
    private void CreateCallout(NetworkFilter.IpFilter instance)
    {
        instance.CreateCallout(
            new DisplayData
            {
                Name = "SyncVPN block dns callout",
